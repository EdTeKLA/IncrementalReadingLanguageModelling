...
def main():
    for data_type in ["word", "pos"]:
        for stimuli in ["L1", "L2", "test"]:
            data_path = f'../../../output/PCFG/parse_{stimuli}_{data_type}_t_0.02.output'
            surprisals = read_roark_surprisals(data_path)
            surprisals = surprisals[surprisals.token != '</s>']
            if data_type == "word":
                for surp in ["surp", "syn_surp", "lex_surp"]:
                    output_path = f'../../../output/PCFG/{stimuli}/{stimuli}_{data_type}_{surp}.csv'
                    surprisals.to_csv(output_path, columns=["token", surp], index=False, header=False)
            else:
                output_path = f'../../../output/PCFG/{stimuli}/{stimuli}_{data_type}_surp.csv'
                surprisals.to_csv(output_path, columns=["token", "surp"], index=False, header=False)
