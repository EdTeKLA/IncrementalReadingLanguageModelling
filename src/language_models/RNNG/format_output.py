...
def get_output(output_path, dataset):
    ...
    for file in result_time_files:
        count += 1
        print(count)
        pattern = dataset + '_time_(.+)_(.+)_(.+)_(.+)_(similar_length|random)'
...
def main():
    dataset = 'wiki'
    for tuning_round in range(3):
        output_path = Path(f'../../../output/RNNG/hyperparameter_tuning/tuning_round_{tuning_round}/')
        new_output_path = Path(f'../../../output/RNNG/hyperparameter_tuning/round_{tuning_round}_wiki_parameter_results.csv')
