...
nrow(data)
data
chinese_vocab = data$Vocab_Comp_C[!is.na(data$Vocab_Comp_C)]
english_vocab = data$Vocab_Comp_E[!is.na(data$Vocab_Comp_E)]
korean_vocab = data$Vocab_Comp_K[!is.na(data$Vocab_Comp_K)]
spanish_vocab = data$Vocab_Comp_S[!is.na(data$Vocab_Comp_S)]
...
kruskal_effsize(data = data, LOR ~ Language)

pairwise.wilcox.test(data$LOR, data$Language, p.adj="holm", exact=F)

chinese_data = subset(data, Language=="Chinese")
korean_data = subset(data, Language=="Korean")
spanish_data = subset(data, Language=="Spanish")
chinese_data
wilcox.test(chinese_data$LOR, korean_data$LOR, exact=F)
wilcox.test(chinese_data$LOR, spanish_data$LOR, exact=F)
wilcox.test(korean_data$LOR, spanish_data$LOR, exact=F)
...
english_data <- subset(competence, Language=="English")
chinese_data <- subset(competence, Language=="Chinese")
korean_data <- subset(competence, Language=="Korean")
spanish_data <- subset(competence, Language=="Spanish")
...
kruskal_effsize(data = competence, Comp ~ Language)
pairwise.wilcox.test(competence$Comp, competence$Language, p.adj="holm", exact=F)


wilcox.test(english_data$Comp, chinese_data$Comp, exact=F)
wilcox.test(english_data$Comp, korean_data$Comp, exact=F)
wilcox.test(english_data$Comp, spanish_data$Comp, exact=F)

wilcox.test(chinese_data$Comp, korean_data$Comp, exact=F)
wilcox.test(chinese_data$Comp, spanish_data$Comp, exact=F)

wilcox.test(korean_data$Comp, spanish_data$Comp, exact=F)


wilcoxonZ(english_data$Comp, chinese_data$Comp, exact=F)/sqrt((nrow(english_data)+nrow(chinese_data)))
wilcoxonZ(english_data$Comp, korean_data$Comp, exact=F)/sqrt((nrow(english_data)+nrow(korean_data)))
wilcoxonZ(english_data$Comp, spanish_data$Comp, exact=F)/sqrt((nrow(english_data)+nrow(spanish_data)))

wilcoxonZ(chinese_data$Comp, korean_data$Comp, exact=F)/sqrt((nrow(chinese_data)+nrow(korean_data)))
wilcoxonZ(chinese_data$Comp, spanish_data$Comp, exact=F)/sqrt((nrow(chinese_data)+nrow(spanish_data)))

wilcoxonZ(korean_data$Comp, spanish_data$Comp, exact=F)/sqrt((nrow(korean_data)+nrow(spanish_data)))




# Vocabulary
kruskal.test(Vocab ~ Language, data = competence)
kruskal_effsize(data = competence, Vocab ~ Language)
pairwise.wilcox.test(competence$Vocab, competence$Language, p.adj="holm", exact=F)


wilcox.test(english_data$Vocab, chinese_data$Vocab, exact=F)
wilcox.test(english_data$Vocab, korean_data$Vocab, exact=F)
wilcox.test(english_data$Vocab, spanish_data$Vocab, exact=F)

wilcox.test(chinese_data$Vocab, korean_data$Vocab, exact=F)
wilcox.test(chinese_data$Vocab, spanish_data$Vocab, exact=F)

wilcox.test(korean_data$Vocab, spanish_data$Vocab, exact=F)


wilcoxonZ(english_data$Vocab, chinese_data$Vocab, exact=F)/sqrt((nrow(english_data)+nrow(chinese_data)))
wilcoxonZ(english_data$Vocab, korean_data$Vocab, exact=F)/sqrt((nrow(english_data)+nrow(korean_data)))
wilcoxonZ(english_data$Vocab, spanish_data$Vocab, exact=F)/sqrt((nrow(english_data)+nrow(spanish_data)))

wilcoxonZ(chinese_data$Vocab, korean_data$Vocab, exact=F)/sqrt((nrow(chinese_data)+nrow(korean_data)))
wilcoxonZ(chinese_data$Vocab, spanish_data$Vocab, exact=F)/sqrt((nrow(chinese_data)+nrow(spanish_data)))

wilcoxonZ(korean_data$Vocab, spanish_data$Vocab, exact=F)/sqrt((nrow(korean_data)+nrow(spanish_data)))
