...
save_path = paste("/Users/shannon/Documents/IncrementalReadingLanguageModelling/output/GAMMs/figures/smooth_" ,type,"_base_model_", base_model_num, "_plots_with_residuals", sep="")
...
for (language in list("E", "C", "K", "S")) {
  for (model in 1:9) {
    ...
    if (model == 2 | model == 5 | model == 7) {
      # ,fun = exponent
      draw(b, rug= FALSE, select=select_opt, residuals=TRUE, resid_col="#cccccc", smooth_col = colours[[model]], ci_col = colours[[model]]) + labs(x=x_labels[model]) + 
        ylim(y_min, y_max) +
    ...
    ggsave(filename =paste(language,"_", model,".png", sep=""), path = save_path, width=2.5, height=2.5, device="png")
