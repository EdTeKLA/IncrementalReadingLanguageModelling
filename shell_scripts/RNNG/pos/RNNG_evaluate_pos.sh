...
do
   STIMULI_PATH="IncrementalReadingLanguageModelling/data/stimuli/${STIMULI[i]}/stimuli_${STIMULI[i]}_pos.txt"
   SURPRISAL_PATH="surprisals_${STIMULI[i]}_pos.txt"
   TREE_PATH="trees_${STIMULI[i]}_pos.parsed"
...
STIMULI_PATH="IncrementalReadingLanguageModelling/data/wiki/sequences/pos/wiki_test_pos.txt"
SURPRISAL_PATH="surprisals_test_pos.txt"
TREE_PATH="trees_test_pos.parsed"
python "$SRC_PATH"  --test_file "$STIMULI_PATH"  --model_file "$MODEL_PATH" --batch_size 10 --block_size 100 --beam_size 400 --word_beam_size 40 --shift_size 4  --gpu 0 --lm_output_file "$SURPRISAL_PATH" > "$TREE_PATH"
